...
// +----------------------------------------------------------------------
class Forum_Util_Captcha extends Typecho_Widget{
    
    protected $config =	array(
        'seKey'     =>  'SISOME.COM',   // 验证码加密密钥
    ...
    public function render($key=''){
        ...
        $secode['verify_time'] = $this->_nowTime->timeStamp;  // 验证码创建时间
        Forum_Util_Session::set($key, $secode);
    ...
    public function check($code,$key=''){
        $key = $this->authcode($this->seKey).$key;
        // 验证码不能为空
        $secode = Forum_Util_Session::get($key);
        if(empty($code) || empty($secode)) {
            return false;
        }
        // session 过期
        if($this->_nowTime->timeStamp - $secode['verify_time'] > $this->expire) {
            Forum_Util_Session::delete($key);
            return false;
        }
        
        if($this->authcode(strtoupper($code)) == $secode['verify_code']) {
            $this->reset && Forum_Util_Session::delete($key);
            return true;
        }
