...
// +----------------------------------------------------------------------
class Forum_Options extends Widget_Abstract_Options implements Widget_Interface_Do{
    ...
    public function form(){
		...
		//用户积分
		$credits_register = new Typecho_Widget_Helper_Form_Element_Text('credits_register', NULL, $this->options->credits_register, _t('注册积分'),_t('用户注册后默认的积分'));
		$credits_register->input->setAttribute('class', 'w-60');
        $form->addInput($credits_register);
		
		$credits_login = new Typecho_Widget_Helper_Form_Element_Text('credits_login', NULL, $this->options->credits_login, _t('登录积分'),_t('每日登录获取的积分'));
		$credits_login->input->setAttribute('class', 'w-60');
        $form->addInput($credits_login);
		
		$credits_post = new Typecho_Widget_Helper_Form_Element_Text('credits_post', NULL, $this->options->credits_post, _t('发布主题'),_t('用户发布主题加上或减少的积分'));
		$credits_post->input->setAttribute('class', 'w-60');
        $form->addInput($credits_post);
		
		$credits_reply = new Typecho_Widget_Helper_Form_Element_Text('credits_reply', NULL, $this->options->credits_reply, _t('发表回复'),_t('用户发表回复加上或减少的积分'));
		$credits_reply->input->setAttribute('class', 'w-60');
        $form->addInput($credits_reply);
		
		$credits_invite = new Typecho_Widget_Helper_Form_Element_Text('credits_invite', NULL, $this->options->credits_invite, _t('邀请注册'),_t('邀请者和被邀请者所奖励的积分'));
