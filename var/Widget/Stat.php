...
class Widget_Stat extends Typecho_Widget
{
     ...
     * @return integer
     */
    protected function ___favoriteNodesNum(){
        return $this->db->fetchObject($this->db->select(array('COUNT(fid)' => 'num'))
            ->from('table.favorites')
            ->where('table.favorites.type = ?', 'tag')
            ->where('table.favorites.uid = ?', $this->user->uid))->num;
    }
    /**
     * 获取当前用户收藏文章数
     *
     * @return integer
     */
    protected function ___favoritePostsNum(){
        return $this->db->fetchObject($this->db->select(array('COUNT(fid)' => 'num'))
            ->from('table.favorites')
            ->where('table.favorites.type = ?', 'post')
            ->where('table.favorites.uid = ?', $this->user->uid))->num;
    }
    /**
     * 获取标签数目
    ...
    protected function ___myPublishedCommentsNum()
    {
        ...
        return $this->db->fetchObject($this->db->select(array('COUNT(coid)' => 'num'))
                    ...
                    ->where('table.comments.authorId = ?', $this->user->uid))->num;
    }
